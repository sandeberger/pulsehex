...
import { PAL_CLOCK, PERIOD_TABLE, SINE_TBL, periodToNoteIdx } from '../../modplayer/js/constants.js';
...
export class GamePlayer {
  ...
  load(doc) {
    this.stop();
    this.mod = doc;
    this._setupChannels(doc.channels);
    this._resetState();
    this.audioBuffers = doc.sampleData.map((d) => {
  ...
  _processTick(time) {
    const pi = this.mod.patternOrder[this.orderIdx];
    const pat = this.mod.patterns[pi];
    if (this.tick === 0) {
      if (pat && pat[this.row]) this._processRow(pat[this.row], time);
      this.bus.emit('row', { row: this.row, order: this.orderIdx, pattern: pi, time });
      if (this.row % this.speed === 0) {
        this.bus.emit('tick', { row: this.row, bpm: this.bpm, time });
      }
    }
    ...
    if (this.tick >= this.speed) {
      this.tick = 0;
      const prevOrder = this.orderIdx;
      this.row++;
      if (this.breakPat) {
        this.orderIdx = this.jumpOrd !== -1 ? this.jumpOrd : this.orderIdx + 1;
        this.row = this.nextRow;
        this.breakPat = false;
        this.jumpOrd = -1;
        this.nextRow = 0;
      } else if (this.jumpOrd !== -1) {
        this.orderIdx = this.jumpOrd;
        this.row = 0;
        this.jumpOrd = -1;
      }
      if (this.row >= 64) { this.row = 0; this.orderIdx++; }
      if (this.orderIdx >= this.mod.songLen) {
        if (this.onSongEnd) { this.stop(); this.onSongEnd(); this.bus.emit('songEnd'); return; }
        this.orderIdx = 0;
        this.bus.emit('songEnd');
      }
      if (this.orderIdx !== prevOrder) {
        this.bus.emit('patternChange', { order: this.orderIdx, pattern: this.mod.patternOrder[this.orderIdx] });
      }
      if (this.bpm !== this._prevBpm) {
        this.bus.emit('bpmChange', { bpm: this.bpm });
        this._prevBpm = this.bpm;
      }
    }
  }
    ...
    rowData.forEach((note, ci) => {
      ...
      if (note.period > 0) {
        if (eff === 3 || eff === 5) { ch.portDest = note.period; }
          ...
          ch.arpBase = note.period;
          ch.vibPos = 0;
          ch.tremPos = 0;
          if (!this.muted[ci]) {
            this._trigger(ch, time, eff === 9 ? par : 0);
            this.bus.emit('noteOn', { channel: ci, sample: note.sample, period: note.period, time });
          }
        }
      }
      switch (eff) {
